...
class TelegramClient:
    ...
    async def _request(self, method: str, endpoint: str, **kwargs) -> Any:
        ...
        if response.status_code in (400, 401, 403, 404):
            ...
            raise TelegramAPIError(f"API Error {response.status_code}: {description}", status_code=response.status_code, response_data=data)
        ...
        try:
            ...
            raise TelegramAPIError(f"HTTP Error {e.response.status_code}", status_code=e.response.status_code)
            
        # Parse JSON and verify 'ok'
        data = response.json()
        if not data.get("ok"):
            description = data.get("description", "Unknown error")
            raise TelegramAPIError(f"Telegram returned ok=false: {description}", response_data=data)
...
if __name__ == "__main__":
    ...
    async def run_test():
        ...
        try:
            ...
            logger.info("Polling getUpdates for 5 seconds (send a message to the bot now!)...")
